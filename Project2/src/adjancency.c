...
ListNode * InsertAdjacency(int vertices, int x, int y, int z, char type, ListNode* listhead, Array decoder)
{
     ...
     *****************************************************************************/
       ...
       Array InitAdj(int vertices, char *** matrix, Array decoder, int col, int row)
    {
        Array adj;
        int i, x, y, z;
        char type;
        ListNode * listhead = ListInit();
        ...
        for(i=0; i<vertices; i++)
        {
            ...
            ModifyArrayNode(i, (Item) listhead, adj);
        }
        return adj;
    }
...
void freeAdj(Array adj, int size)
{
  int i;

  for( i = 0; i < size; i++)
  {
		freeLinkedList( (ListNode *) GetArrayNode(i, adj) ); /*Frees the adjancency list for each vertice */
  }

  free(adj); /*Frees the Array */
}
